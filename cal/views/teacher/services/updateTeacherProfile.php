<?php
    ...
    if(checkRequest()) {
        ...
        $sql = "UPDATE users SET name = '$name', phone = '$phone', addr = '$addr', desg = '$desg' WHERE uid = $uid";

        if($conn->query($sql)) {
           $_SESSION["user"]["name"] = $name;
           $_SESSION["user"]["phone"] = $phone;
           $_SESSION["user"]["addr"] = $addr;
           $_SESSION["user"]["desg"] = $desg;
           $obj->result = true;
        } else {
            $obj->result = false;
    ...
    function checkRequest() {
		...
		if($_SERVER["REQUEST_METHOD"] == "POST") {
            if(isset($_POST["name"]) && isset($_POST["phone"]) && isset($_POST["addr"]) && isset($_POST["desg"])) {
                $result = true;
            }
        }
